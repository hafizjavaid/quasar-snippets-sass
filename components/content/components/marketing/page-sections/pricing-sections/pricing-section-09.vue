<template>
    <div class="full-width bg-grey-3">
        <div style="max-width: 1200px;" class="q-mx-auto q-pa-xl">
            <div class="row q-col-gutter-md ">
                ...
                <div class="col-12 col-md-6 flex column justify-between q-gutter-md">
                    <q-card v-for="(item, i) in plans" :key="i" boredered class="flex items-center q-pa-lg" flat
                        ...
                        <div :class="[$q.screen.gt.sm ? 'text-h3' : 'text-h5']"
                            class="flex text-h3 q-my-none justify-end" style="flex-grow: 1;">
                            <template v-if="plan == 'monthly'">
                                <small :class="[$q.screen.gt.sm ? 'text-h5' : 'text-body2']"
                                    class="q-my-none">$</small>{{ item.cost }}
                            </template>
                            <template v-else>
                                <small :class="[$q.screen.gt.sm ? 'text-h5' : 'text-body2']"
                                    class="q-my-none">$</small>{{ item.yearlyCost }}
                            </template>
