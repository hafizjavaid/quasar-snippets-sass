<!-- Pricing Section 05 -->
::ContentCard{title="Pricing Section 05" slug="pricing-section-05"}

#preview
::pricing-section-05
::

#codebase
::CodeView

```html
<template>
    <div class="img-gradient q-pa-md q-pa-md-lg">
        <div style="max-width: 700px;" class="q-mx-auto text-center q-mb-lg q-mb-md-xl">
            <p class="text-weight-medium text-primary">Pricing</p>
            <h4 :class="[$q.screen.gt.sm ? 'text-h4' : 'text-h6']" class="text-weight-bold q-mt-none q-mb-md">
                Pricing plans for teams of all sizes
            </h4>
            <p class="text-subtitle1">
                Distinction and no one freed him, and not by some labor.
                Often and as if it were just a way of wanting to not take pleasure in it.
                I will explain it as snacks.
            </p>
        </div>

        <div style="max-width: 1000px;" class="q-mx-auto">
            <div class="row items-end q-mt-xl q-col-gutter-lg">
                <div v-for="(item, i) in plans" :key="i" class="col-12 col-md-6">
                    <q-card style="height: 500px; border-radius: 24px;"
                        class="q-pa-md q-pa-md-lg relative-position shadow-6 q-mx-auto">
                        <p class="text-subtitle2 text-weight-bold q-mb-md">
                            {{ item.title }}
                        </p>
                        <div :class="[$q.screen.gt.sm ? 'text-h4' : 'text-h6']" class="q-mb-md text-weight-bold">
                            ${{ item.cost }}
                            <small class="text-body2">
                                /month
                            </small>
                        </div>
                        <p class="text-subtitle2  q-mb-md">
                            {{ item.subtitle }}
                        </p>
                        <div class="flex flex-center">
                            <q-btn no-caps dense style="width: 100%;" class="q-mx-auto q-mb-md" color="primary"
                                label="Get started today" unelevated />
                        </div>

                        <q-item style="min-height: unset" v-for="(feature, _index) in item.features" :key="_index"
                            class="flex items-center q-gutter-x-sm q-px-none text-body2 text-grey-9">
                            <q-icon size="16px" name="mdi-check" />
                            <q-item-section>
                                {{ feature }}
                            </q-item-section>
                        </q-item>
                    </q-card>
                </div>
                <div class="col-12">
                    <q-card style="border-radius: 20px" bordered flat class="q-pa-lg">
                        <div class="row">
                            <div class="col-12 col-md-8">
                                <p class="q-mb-md text-weight-bold text-primary"> Discounted </p>
                                <p class="text-subtitle2 text-weight-normal">
                                    Dolor dolores repudiandae doloribus. Rerum sunt aut eum. Odit omnis non voluptatem
                                    sunt eos
                                    nostrum.
                                </p>
                            </div>
                            <div class="col-12 col-md-4 text-right">
                                <q-btn class="" no-caps padding="7px 12px" style="border-radius: 8px;" outline
                                    color="primary">
                                    <template #default>
                                        <div class="text-primary q-mr-sm">Buy discounted license</div>
                                        <q-icon color="primary" name="mdi-arrow-right"> </q-icon>
                                    </template>
                                </q-btn>
                            </div>
                        </div>
                    </q-card>
                </div>
            </div>

        </div>
    </div>
</template>

<script setup lang="ts">
import { useQuasar } from 'quasar'
const $q = useQuasar();

const plans = [
    {
        title: 'Hobby',
        subtitle: 'Modi dolorem expedita deleniti. Corporis iste qui inventore pariatur adipisci vitae.',
        cost: 49,
        features: [
            '5 products',
            'Up to 1,000 subscribers',
            'Basic analytics',
            '48-hour support response time',
        ],
    },
    {
        title: 'Team',
        subtitle: 'Explicabo quo fugit vel facere ullam corrupti non dolores. Expedita eius sit sequi.',
        cost: 79,
        features: [
            'Unlimited products',
            'Unlimited subscribers',
            'Advanced analytics',
            '1-hour, dedicated support response time',
            'Marketing automations',
            'Custom reporting tools',
        ],
    },
]
</script>

<style scoped>
.img-gradient {

    background: linear-gradient(to bottom, rgba(255, 255, 255, .8), rgba(255, 255, 255, 1) 80%),
        url('https://images.unsplash.com/photo-1527689368864-3a821dbccc34?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D');
    background-size: cover;
    background-position: center;
}
</style>

```

::